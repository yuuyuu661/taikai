  ...
  const editForm = `
    <form class="editbox only-admin" data-edit="${t.id}" style="display:none">
      <div class="row">
        ...
        <div><label>リーダーP1</label><select name="leader_poke1_url"></select></div>
        <div><label>リーダーP2</label><select name="leader_poke2_url"></select></div>
        <div><label>リーダーP3</label><select name="leader_poke3_url"></select></div>
      </div>
      <hr>
      <div class="row">
        ${t.members.slice(0,4).map((m,i)=>`
          ...
          <div><label>P1</label><select name="m${i+1}_poke1_url"></select></div>
          <div><label>P2</label><select name="m${i+1}_poke2_url"></select></div>
          <div><label>P3</label><select name="m${i+1}_poke3_url"></select></div>
        `).join('')}
      </div>
...
async function loadTeams() {
  ...
  el.querySelectorAll('.team-card').forEach(card => {
    ...
    editBtn.addEventListener('click', async () => {
      ...
      // セレクトに画像候補を流し込んで既存値をセット
      const fill = (sel, value) => {
        sel.innerHTML = '<option value="">— 選択 —</option>' + imageOptions.map(p=>`<option value="${p}">${p.replace('/images/','')}</option>`).join('');
        sel.value = value||'';
      };
      fill(form.querySelector('select[name="leader_poke1_url"]'), data.leader_poke1_url);
      fill(form.querySelector('select[name="leader_poke2_url"]'), data.leader_poke2_url);
      fill(form.querySelector('select[name="leader_poke3_url"]'), data.leader_poke3_url);
      (data.members||[]).slice(0,4).forEach((m,i)=>{
        fill(form.querySelector(`select[name="m${i+1}_poke1_url"]`), m.poke1_url);
        fill(form.querySelector(`select[name="m${i+1}_poke2_url"]`), m.poke2_url);
        fill(form.querySelector(`select[name="m${i+1}_poke3_url"]`), m.poke3_url);
      });
      reflectAdmin();
    });
