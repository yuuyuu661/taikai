...
app.put('/api/teams/:id', requireAdmin, upload.fields([
  ...
  try {
    ...
    // メンバーは team_id 昇順で 1..4 を紐付ける（不足はスキップ）
    const { rows: mems } = await pool.query('SELECT * FROM team_members WHERE team_id=$1 ORDER BY id', [id]);
    const up = async (idx, fields, iconFieldName) => {
      ...
      await pool.query(
        `UPDATE team_members SET 
         ...
         WHERE id=$7`,
        [fields.name||null, f?f.buffer:null, f?f.mimetype:null, fields.p1||null, fields.p2||null, fields.p3||null, m.id]
      );
    };
