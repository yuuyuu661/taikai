...
app.get('/api/teams/:id', async (req,res)=>{
  ...
  const { rows: trows } = await pool.query('SELECT id, team_name, leader_name, leader_poke1_url, leader_poke2_url, leader_poke3_url, (leader_icon IS NOT NULL) AS has_leader_icon FROM teams WHERE id=$1',[id]);
  if(!trows[0]) return res.status(404).json({error:'not found'});
  const { rows: mems } = await pool.query('SELECT id, team_id, member_name, poke1_url, poke2_url, poke3_url, (member_icon IS NOT NULL) AS has_member_icon FROM team_members WHERE team_id=$1 ORDER BY id',[id]);
