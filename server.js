...
app.delete('/api/teams/:id', requireAdmin, async (req, res) => {
  try {
    const id = Number(req.params.id);
    await pool.query('DELETE FROM teams WHERE id=$1', [id]); // team_membersはON DELETE CASCADE
    // matches側は ON DELETE SET NULL なので、孤立試合は残る（必要ならクリーンアップ処理を追加）
    res.json({ ok:true });
  } catch (e) {
    console.error(e);
