...
// ====== 静的ファイルを配信（重要: public/images をそのまま配信） ======
...
function chunkInto(arr, size) { const r=[]; for (let i=0;i<arr.length;i+=size) r.push(arr.slice(i,i+size)); return r; }
function generateRoundRobin(teamIds) {
  const t=[...teamIds]; if (t.length%2===1) t.push(null);
  const n=t.length, rounds=n-1, half=n/2, pairs=[];
...
async function computeStandings() {
  ...
  for (const g of games) {
    const a = map.get(g.team_a); const b = map.get(g.team_b); if (!a||!b) continue;
    a.played++; b.played++;
    ...
    for (const f of ['most_score_team_id','most_kills_team_id','most_assists_team_id']) {
      if (g[f]) { const t = map.get(g[f]); if (t) { t.pts += 1; t.bonus += 1; } }
    }
  }
...
app.get('/api/images', (req, res) => {
  try {
    ...
    console.error('read images error', e);
    res.json([]);
  }
});
...
app.get('/api/teams/:id', async (req, res) => {
  ...
  const { rows: trows } = await pool.query('SELECT * FROM teams WHERE id=$1', [id]);
  if (!trows[0]) return res.status(404).json({ error:'not found' });
  const { rows: mems } = await pool.query('SELECT * FROM team_members WHERE team_id=$1 ORDER BY id', [id]);
  ...
  try {
    ...
    await pool.query(
      `UPDATE teams SET 
       ...
       WHERE id=$8`,
      [team_name||null, leader_name||null, leaderFile?leaderBuf:null, leaderFile?leaderMime:null,
       leader_poke1_url||null, leader_poke2_url||null, leader_poke3_url||null, id]
    );
...
app.post('/api/schedule/generate', requireAdmin, async (req, res) => {
  try {
    ...
    let round=1;
    for (const group of packed) { let slot=1; for (const [a,b] of group) {
      await pool.query(`INSERT INTO matches (round, slot, team_a, team_b) VALUES ($1,$2,$3,$4)`, [round, slot, a, b]); slot++;
    } round++; }
...
app.get('/api/schedule', async (req, res) => {
  ...
  `);
  const grouped = {}; for (const r of rows) { grouped[r.round] = grouped[r.round] || []; grouped[r.round].push(r); }
  res.json(grouped);
});
app.post('/api/match/:id/result', requireAdmin, async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE matches
      ...
      WHERE id=$5`, [winner_team_id, most_score_team_id, most_kills_team_id, most_assists_team_id, id]);
    res.json({ ok: true });
  } catch (e) { console.error(e); res.status(500).json({ error: 'failed to submit result' }); }
});
app.get('/api/standings', async (req, res) => { const table = await computeStandings(); res.json(table); });
